...
void Client::write(Packet *packet) {
    _io_service.post(
            [this, packet]() {
                bool write_in_progress = !_writePacketQueue.empty();
                _writePacketQueue.push_back(packet);
...
void Client::do_write() {
    boost::asio::async_write(_socket,
                             boost::asio::buffer(_writePacketQueue.front()->data(),
                                                 _writePacketQueue.front()->length()),
                             [this](boost::system::error_code ec, std::size_t /*length*/) {
                                 if (!ec) {
                                     _writePacketQueue.pop_front();
                                     if (!_writePacketQueue.empty()) {
                                         do_write();
                                     }
                                 } else {
                                     _socket.close();
