...
std::string CommonCrypto::encryptAES(const std::string &message)
{
    ...
    std::string encryptedMessage = std::string('\0', message.size() + AES_BLOCK_SIZE);
    ...
    if(!EVP_EncryptUpdate(_encryptContextAES, (unsigned char *)encryptedMessage.data(), (int*)&blockLength, (unsigned char *)message.c_str(), message.size()))
        throw CryptoException("Cannot Update AES Encryption");

    encryptedMessageLength += blockLength;

    if(!EVP_EncryptFinal_ex(_encryptContextAES, (unsigned char *)encryptedMessage.data() + encryptedMessageLength, (int*)&blockLength))
...
std::string CommonCrypto::decryptAES(const std::string &encryptedMessage) {
    ...
    std::string decryptedMessage = std::string('\0', encryptedMessage.size());
    ...
    if(!EVP_DecryptUpdate(_decryptContextAES, (unsigned char*)decryptedMessage.data(), (int*)&blockLength, (unsigned char *)encryptedMessage.c_str(), encryptedMessage.size()))
        throw CryptoException("Cannot Update AES Decryption");

    decryptedMessageLength += blockLength;

    if(!EVP_DecryptFinal_ex(_decryptContextAES, (unsigned char*)decryptedMessage.data() + decryptedMessageLength, (int*)&blockLength))
